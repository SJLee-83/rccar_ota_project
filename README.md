...
SSAFY 임베디드 트랙 15기 1학기 관통 PJT. 차량이 출고 후에도 소프트웨어 업데이트로 기능이 확장되는 **SDV(Software Defined Vehicle)** 개념을 RC카 규모로 구현.

- **무선 OTA 업데이트**: 제어 MCU 에 무선 모듈이 없어도 게이트웨이(ESP32)를 거쳐 펌웨어를 무선 배포하고, 바뀐 펌웨어가 주행 동작 변화로 이어지는 것까지 시연
...
부트로더 없이 RA6E1 하드웨어의 Dual-Bank + BankSwap 기능으로 구현. 실행 중인 Active Bank 가 새 펌웨어를 받아 반대쪽 Inactive Bank(alias `0x00200000`)에 기록하고, 검증 성공 시 `R_FLASH_HP_BankSwap()` + Reset 으로 교체.

- 256바이트 청크 전송, RA6E1 에서 메타데이터·청크 CRC32 검증 (불일치 시 거부). 전체 이미지 CRC32 는 ESP32 가 전송 완료 시 대조하되 경고만 기록, RA6E1 은 수신만 하고 미대조
- ARQ 재전송 (NACK `0x1F`, 청크당 최대 20회), Session ID, 바이너리 프로토콜
- 다운로드·검증 실패 시 BankSwap 미수행으로 기존 펌웨어 유지

### 2. HC-SR04 충돌 회피 (OTA 가치 시연용)

OTA 로 펌웨어를 바꾸면 기능이 생기는 것을 보이는 시연 구성.
...
MCUboot 부트로더 갈래는 [rccar-mcuboot-ota](https://github.com/SJLee-83/rccar-mcuboot-ota) 로 분리. 이 저장소는 팀 결과물만 유지.
...
| 무선 전송 | 완성 | 미완 (self-programming 제약) |
| 최종 시연 | 채택 | 미채택 |

MCUboot 방식 상세와 코드는 [rccar-mcuboot-ota](https://github.com/SJLee-83/rccar-mcuboot-ota) 참조.

프로젝트 상세 정리는 [docs/01_팀프로젝트_전체정리.md](docs/01_팀프로젝트_전체정리.md) 참조.
